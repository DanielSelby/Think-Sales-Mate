...
export default async function PayslipsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  let query = supabase
    .from("payslips")
    .select("id, employee_name, employee_number, period_label, pay_period_start, payment_date, net_pay, currency, status, generated_at, department, location_id")
      ...
      <div className="mx-auto max-w-[1680px] space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-[#dce8f2] bg-white p-4 shadow-sm dark:border-ledger-700 dark:bg-ink-900">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-600 text-white"><FileText className="h-6 w-6" /></div>
            <div><div className="mb-1 text-[11px] font-semibold text-blue-600">HRM &amp; Payroll <span className="mx-1 text-slate-300">›</span> Payslip Generated</div><h1 className="text-xl font-bold text-[#12345a] dark:text-white">Payslip Generated</h1><p className="text-xs text-ledger-500">View, manage and print employee payslips.</p></div>
          </div>
          <Link href="/hrm/payroll" className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-blue-700"><FileText className="h-4 w-4" />Generate Payslip</Link>
        </div>
        ...
        <form method="get" className="grid gap-3 rounded-xl border border-[#dce8f2] bg-white p-3 shadow-sm dark:border-ledger-700 dark:bg-ink-900 md:grid-cols-3 xl:grid-cols-6">
          ...
          <div className="flex items-end"><button type="submit" className="h-9 w-full rounded-md bg-blue-600 px-3 text-xs font-semibold text-white hover:bg-blue-700">Apply Filters</button><Link href={clearHref} className="ml-2 inline-flex h-9 items-center whitespace-nowrap rounded-md border border-slate-200 px-3 text-xs font-semibold text-slate-600 hover:bg-slate-50 dark:border-ledger-700 dark:text-slate-300">Clear</Link></div>
        </form>

        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <Kpi icon={<FileText />} label="Total Payslips" value={String(rows.length)} tone="blue" />
          <Kpi icon={<WalletIcon />} label="Total Salary Amount" value={formatCurrency(totalSalary, currency)} tone="green" />
          <Kpi icon={<CalendarDays />} label="Current Month Payroll" value={formatCurrency(currentMonthTotal, currency)} tone="purple" />
          <Kpi icon={<BriefcaseBusiness />} label="Departments" value={String(departmentCount)} tone="orange" />
          <Kpi icon={<Building2 />} label="Branches" value={String(branchCount)} tone="teal" />
        </div>
...
function WalletIcon() { return <WalletCards />; }
function Kpi({ icon, label, value, tone }: { icon: ReactNode; label: string; value: string; tone: "blue" | "green" | "purple" | "orange" | "teal" }) {
  const styles = { blue: "bg-blue-50 text-blue-600", green: "bg-emerald-50 text-emerald-600", purple: "bg-purple-50 text-purple-600", orange: "bg-orange-50 text-orange-600", teal: "bg-teal-50 text-teal-600" };
  return <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-4 shadow-sm dark:border-ledger-700 dark:bg-ink-900"><div className={`flex h-9 w-9 items-center justify-center rounded-full ${styles[tone]}`}>{icon}</div><div><p className="text-[10px] font-semibold text-slate-500">{label}</p><p className="mt-1 text-lg font-bold text-[#12345a] dark:text-white">{value}</p></div></div>;
}
