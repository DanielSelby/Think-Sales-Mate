...
import { AuditVisuals } from "@/components/audit/audit-visuals";
...
function readableValue(value: unknown) {
  ...
  if (typeof value === "object") return Array.isArray(value) ? `${value.length} item${value.length === 1 ? "" : "s"}` : "Updated details";
  return String(value);
}

function changeSummary(record: AuditRecord) {
  const before = record.metadata.previous_values;
  const after = record.metadata.new_values;
  if (!before && !after) return "No before-and-after values were recorded.";
  const beforeObject = before && typeof before === "object" && !Array.isArray(before) ? before as Record<string, unknown> : {};
  const afterObject = after && typeof after === "object" && !Array.isArray(after) ? after as Record<string, unknown> : {};
  const keys = [...new Set([...Object.keys(beforeObject), ...Object.keys(afterObject)])];
  const changes = keys
    .filter((key) => JSON.stringify(beforeObject[key]) !== JSON.stringify(afterObject[key]))
    .slice(0, 4)
    .map((key) => `${labelValue(key)}: ${readableValue(beforeObject[key])} → ${readableValue(afterObject[key])}`);
  return changes.length ? changes.join(" · ") : "Record details changed.";
}

function changeEntries(record: AuditRecord) {
  const before = record.metadata.previous_values;
  const after = record.metadata.new_values;
  const beforeObject = before && typeof before === "object" && !Array.isArray(before) ? before as Record<string, unknown> : {};
  const afterObject = after && typeof after === "object" && !Array.isArray(after) ? after as Record<string, unknown> : {};
  return [...new Set([...Object.keys(beforeObject), ...Object.keys(afterObject)])]
    .filter((key) => JSON.stringify(beforeObject[key]) !== JSON.stringify(afterObject[key]))
    .map((key) => ({ key: labelValue(key), before: readableValue(beforeObject[key]), after: readableValue(afterObject[key]) }));
}
...
export function AuditCenter({ records }: { records: AuditRecord[] }) {
  ...
  const moduleBreakdown = modules.map((name) => ({ name, value: countBy(records, (record) => recordModule(record) === name) })).sort((a, b) => b.value - a.value).slice(0, 6);
  const distribution = moduleBreakdown;
  ...
  return <section className="audit-center space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]"><div className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="font-semibold">Activity Trend</h2><span className="text-xs text-slate-400">Last 14 days · active filters</span></div><div className="mt-4 flex h-32 items-end gap-1">{trend.map((point) => <div key={point.key} className="group flex min-w-0 flex-1 flex-col items-center justify-end"><div title={`${point.label}: ${point.total} events, ${point.failed} failed`} className="w-full rounded-t bg-blue-500 transition hover:bg-blue-700" style={{ height: `${point.total / maxTrend * 100}%`, minHeight: point.total ? 3 : 0 }} /><span className="mt-1 hidden truncate text-[9px] text-slate-400 group-first:block">{point.label}</span></div>)}</div></div><div className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Activity Health</h2><div className="mt-4 space-y-3 text-sm"><p className="flex justify-between"><span>Successful events</span><strong className="text-emerald-600">{records.length - tabCount("failed")}</strong></p><p className="flex justify-between"><span>Security events</span><strong>{tabCount("security")}</strong></p><p className="flex justify-between"><span>Failed attempts</span><strong className="text-red-600">{tabCount("failed")}</strong></p></div></div></div>
    <div className="grid gap-5 lg:grid-cols-2"><div className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Activity by Module</h2><p className="mt-1 text-xs text-slate-500">Which ERP areas generated activity.</p><div className="mt-3 space-y-2">{moduleBreakdown.map((item) => <div key={item.name}><div className="flex justify-between text-xs"><span>{labelValue(item.name)}</span><strong>{item.value}</strong></div><div className="mt-1 h-2 rounded bg-slate-100"><div className="h-2 rounded bg-emerald-500" style={{ width: `${item.value / Math.max(1, ...moduleBreakdown.map((entry) => entry.value)) * 100}%` }} /></div></div>)}</div></div><div className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Activity by Action</h2><p className="mt-1 text-xs text-slate-500">What users or systems did.</p><div className="mt-3 space-y-2">{actionBreakdown.map((item) => <button type="button" key={item.name} onClick={() => { setAction(item.name); setPage(1); }} className="flex w-full justify-between text-left text-xs hover:text-blue-600"><span>{labelValue(item.name)}</span><strong>{item.value}</strong></button>)}</div></div></div>
    ...
    {tab === "data" && <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{[["Total Changes", dataChanges.length], ["Creates", changeCount(/create|add/i)], ["Updates", changeCount(/update|edit|change/i)], ["Deletes", changeCount(/delete|remove/i)], ["High Impact", highImpact.length]].map(([label, value]) => <div key={String(label)} className="rounded-xl border bg-white p-4 shadow-sm"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>)}</div><div className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Change Journal</h2><p className="mt-1 text-xs text-slate-500">Changes are shown in plain language. Select a row for full technical details.</p><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="border-b bg-slate-50 uppercase text-slate-500"><tr>{["Date & Time", "User", "Change", "Record", "What changed", "Status"].map((heading) => <th key={heading} className="px-3 py-3">{heading}</th>)}</tr></thead><tbody className="divide-y">{dataChanges.slice(0, 25).map((record) => <tr key={record.id} onClick={() => setSelected(record)} className="cursor-pointer hover:bg-blue-50"><td className="px-3 py-3 text-slate-500">{new Date(record.created_at).toLocaleString()}</td><td className="px-3 py-3 font-medium">{record.actor_name}</td><td className="px-3 py-3 font-semibold">{labelValue(record.action)}</td><td className="px-3 py-3">{labelValue(record.entity_type)} {record.entity_id ? `#${record.entity_id.slice(0, 8)}` : ""}</td><td className="max-w-[420px] px-3 py-3 text-slate-600">{changeSummary(record)}</td><td className="px-3 py-3">{recordStatus(record)}</td></tr>)}{!dataChanges.length && <tr><td colSpan={6} className="p-8 text-center text-slate-500">No persisted data changes match the current filters.</td></tr>}</tbody></table></div></div></div>}
    ...
    {selected && <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" onClick={() => setSelected(null)}><div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded-xl bg-white p-5 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><h2 className="text-lg font-bold">Audit record details</h2><p className="mt-1 text-xs text-slate-500">{new Date(selected.created_at).toLocaleString()}</p></div><button type="button" onClick={() => setSelected(null)} className="text-xl text-slate-400">x</button></div><dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-slate-500">Event ID</dt><dd className="font-mono text-xs">{selected.id}</dd></div>    <div><dt className="text-xs text-slate-500">User / role</dt><dd>{selected.actor_name} / {recordRole(selected)}</dd></div><div><dt className="text-xs text-slate-500">Action</dt><dd>{labelValue(selected.action)}</dd></div><div><dt className="text-xs text-slate-500">Module / entity</dt><dd>{labelValue(selected.entity_type)} / {selected.entity_id ?? "—"}</dd></div><div><dt className="text-xs text-slate-500">Branch / IP address</dt><dd>{textValue(selected.metadata.branch_name) || textValue(selected.metadata.branch_id) || "—"} / {textValue(selected.metadata.ip_address) || "—"}</dd></div><div><dt className="text-xs text-slate-500">Status / severity</dt><dd className="capitalize">{recordStatus(selected)} / {recordSeverity(selected)}</dd></div><div><dt className="text-xs text-slate-500">Description</dt><dd>{textValue(selected.metadata.description) || `${labelValue(selected.action)} on ${labelValue(selected.entity_type)}`}</dd></div><div><dt className="text-xs text-slate-500">User agent</dt><dd className="break-all">{textValue(selected.metadata.user_agent) || "Not captured"}</dd></div><div className="sm:col-span-2"><dt className="text-xs text-slate-500">Change summary</dt><dd className="mt-1 rounded-lg bg-blue-50 p-3 text-sm text-slate-700">{changeSummary(selected)}</dd></div><div className="sm:col-span-2"><dt className="text-xs text-slate-500">Before / after values</dt><dd className="mt-1 overflow-hidden rounded-lg border"><div className="grid grid-cols-3 border-b bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-500"><span>Field</span><span>Before</span><span>After</span></div>{changeEntries(selected).map((change) => <div key={change.key} className="grid grid-cols-3 gap-2 border-b px-3 py-2 text-xs last:border-0"><strong>{change.key}</strong><span className="text-red-700">{change.before}</span><span className="text-emerald-700">{change.after}</span></div>)}{!changeEntries(selected).length && <p className="p-3 text-xs text-slate-500">No individual field changes were recorded.</p>}</dd></div><div className="sm:col-span-2"><details><summary className="cursor-pointer text-xs text-slate-500">Show technical metadata</summary><pre className="mt-1 overflow-auto rounded-lg bg-slate-50 p-3 text-xs">{JSON.stringify(selected.metadata, null, 2)}</pre></details></div></dl></div></div>}
  </section>;
}
