...
export default function PlatformAdminConsole({
  ...
  const usageTotals = {
    ...
    purchases: billing.filter((item) => item.status !== "void").length,
  ...
  const moduleUsage = [
    ["Sales", usageTotals.sales],
    ["Purchases", usageTotals.purchases],
    ["AI Assistant", usage.reduce((sum, item) => sum + Number(item.ai_usage || 0), 0)],
    ["API", usageTotals.api],
  ];
