...
export async function POST(request: Request) {
  ...
  let { data, error } = await supabase.from("platform_organization_features").upsert(values, { onConflict: "organization_id,module" }).select("organization_id, module, enabled, access_mode").single();
  if (error && /permission_options|schema cache/i.test(error.message)) {
    const fallback = { ...values };
    delete (fallback as { permission_options?: Record<string, boolean> }).permission_options;
    ({ data, error } = await supabase.from("platform_organization_features").upsert(fallback, { onConflict: "organization_id,module" }).select("organization_id, module, enabled, access_mode").single());
  }
