...
export default async function CashClosingPage({ searchParams }: { searchParams?: Promise<CashClosingSearchParams> }) {
  ...
  const [closingResult, { data: locations }, { data: currencyRow }, { data: currencySettings }, { data: companyProfile }, { data: members }, summary] = await Promise.all([
    (() => {
      let query = db.from("cash_closings").select("id, location_id, closing_date, shift, opening_cash, cash_sales, cash_receipts, cash_customer_payments, cash_e_cash, cash_refunds, cash_expenses, deposits, withdrawals, actual_cash, expected_cash, variance, classification, status, approval_required, variance_reason, created_at, created_by", { count: "exact" }).eq("org_id", ctx.orgId).gte("closing_date", dateFrom || "1900-01-01").lte("closing_date", dateTo || "2999-12-31");
    ...
    calculateExpectedCash(today, selectedLocationId, "full_day", effectiveUserId || null).catch(() => ({ opening: 0, sales: 0, debt: 0, customerPayments: 0, electronic: 0, refunds: 0, expenses: 0, deposits: 0, withdrawals: 0, expected: 0 })),
  ]);
  const closings = closingResult.error
    ? (await (() => {
        let query = db.from("cash_closings").select("id, location_id, closing_date, opening_cash, cash_sales, cash_receipts, cash_customer_payments, cash_e_cash, cash_refunds, cash_expenses, deposits, withdrawals, actual_cash, expected_cash, variance, classification, status, variance_reason, created_at, created_by").eq("org_id", ctx.orgId);
        if (ctx.isBranchScoped) query = query.in("location_id", ctx.allowedLocationIds);
        if (effectiveUserId) query = query.eq("created_by", effectiveUserId);
        return query.order("created_at", { ascending: false }).range((pageNumber - 1) * pageSize, pageNumber * pageSize - 1);
      })()).data
  ...
  const cards = [
    ...
    ["MoMo / E-Cash Sales", summary.electronic, "bg-signal-soft text-signal", Coins],
      ...
      <div className="mx-auto max-w-[1680px] space-y-5">
        ...
        {selectedClosing && <section className="rounded-xl border border-[#dce8f2] bg-white p-5 shadow-sm dark:border-ledger-700 dark:bg-ink-900">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6 text-xs">
            {([["Expected", selectedClosing.expected_cash], ["Actual", selectedClosing.actual_cash], ["Variance", selectedClosing.variance], ["Cash sales", selectedClosing.cash_sales], ["Customer Debt", selectedClosing.cash_receipts], ["Customer Payment (Cash)", selectedClosing.cash_customer_payments], ["MoMo / E-cash", selectedClosing.cash_e_cash], ["Expenses", selectedClosing.cash_expenses], ["Refund / Return Sales", selectedClosing.cash_refunds]] as const).map(([label, value]) => <div key={label} className="rounded-lg bg-[#f5f9fc] p-3"><p className="text-ledger-500">{label}</p><strong className="mt-1 block text-[#17385d]">{formatCurrencyAmount(Number(value ?? 0), currencyConfig)}</strong></div>)}
          </div>
        ...
        <CashClosingReport closings={reportClosings} currency={currencyConfig} organizationName={companyProfile?.company_name ?? ctx.orgName} logoUrl={companyProfile?.logo_url ?? null} />
        </>}
        {tab === "analytics" && <div>
