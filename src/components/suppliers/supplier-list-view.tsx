  ...
  return (
    <div className="rounded-2xl border border-ledger-100 bg-white p-5 shadow-card dark:border-ledger-700 dark:bg-ink-900">
      <div className="flex items-center gap-3.5">
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${tones[tone]}`}>{icon}</div>
        <div className="min-w-0">
          <p className="truncate text-[11px] font-semibold uppercase tracking-wide text-ledger-400">{label}</p>
          <p className="mt-1 font-display text-2xl font-bold leading-none tracking-tight text-ink-900 dark:text-white">
            {value}
          </p>
        </div>
      </div>
      <p className="mt-3 text-[10px] text-ledger-400">Current supplier history records</p>
    </div>
  );
}
