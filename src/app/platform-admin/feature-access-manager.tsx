...
export function FeatureAccessManager({ organizations, plans, features, auditLogs }: { organizations: Organization[]; plans: Plan[]; features: FeatureRecord[]; auditLogs: Array<{ organization_id: string | null; module: string; created_at: string }> }) {
  ...
  const renderFeature = (moduleName: string, name: string, depth = 0) => {
    ...
    return <div key={key} className={`grid grid-cols-[minmax(180px,1fr)_repeat(3,70px)_repeat(2,72px)] items-center border-b px-3 py-2 text-xs ${depth ? "bg-slate-50/60" : ""}`}><span className={depth ? "pl-5 text-slate-600" : "font-semibold text-slate-800"}>{name}</span>{(["enabled", "disabled", "read_only"] as AccessMode[]).map((mode) => <label key={mode} className="flex justify-center"><input type="radio" name={key} checked={(record?.access_mode ?? "disabled") === mode} onChange={() => updateDraft(key, mode)} /></label>)}{options.map((option) => <label key={option} className="flex justify-center"><input type="checkbox" checked={Boolean(optionValues[option])} onChange={(event) => updateDraft(key, record?.access_mode ?? "disabled", { ...optionValues, [option]: event.target.checked })} /></label>)}</div>;
  };
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_250px]"><aside className="rounded-xl border bg-white p-3 shadow-sm"><h3 className="font-semibold">Modules</h3><div className="mt-3 space-y-1">{visibleModules.map((module) => <button type="button" key={module.name} onClick={() => setActiveModule(module.name)} className={`flex w-full items-center justify-between rounded-lg px-3 py-3 text-left text-xs ${activeModule === module.name ? "bg-blue-600 font-semibold text-white" : "hover:bg-slate-50"}`}><span>{module.name}</span><span>{featureKeys(module).length}</span></button>)}</div></aside>
      <div className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><div><h3 className="font-semibold">{active.name}</h3><p className="text-xs text-slate-500">{selectedPlan ? `${selectedPlan.name} plan · ${selectedPlan.included_modules?.includes(active.name) ? "Included" : "Override available"}` : "Select a subscription plan"}</p></div><div className="flex gap-2"><button type="button" onClick={() => setModuleMode(active, "enabled")} className="rounded border px-2 py-1 text-[11px]">Enable module</button><button type="button" onClick={() => setModuleMode(active, "disabled")} className="rounded border px-2 py-1 text-[11px]">Disable module</button><button type="button" onClick={() => setModuleMode(active, "read_only")} className="rounded border px-2 py-1 text-[11px]">Read only</button></div></div><div className="grid grid-cols-[minmax(180px,1fr)_repeat(3,70px)_repeat(2,72px)] border-b bg-slate-50 px-3 py-2 text-[10px] font-semibold uppercase text-slate-500"><span>Feature / Page</span><span className="text-center">Enabled</span><span className="text-center">Disabled</span><span className="text-center">Read only</span><span className="text-center">Approval</span><span className="text-center">Hidden</span></div>{active.features.map((feature) => <div key={feature.name}>{renderFeature(active.name, feature.name)}{feature.children?.map((child) => renderFeature(active.name, child, 1))}</div>)}</div>
      <aside className="space-y-4"><div className="rounded-xl border bg-white p-4 shadow-sm"><h3 className="font-semibold">Permission Presets</h3>{[["enabled", "Full Access"], ["read_only", "Read Only"], ["disabled", "Limited Access"]].map(([mode, label]) => <button type="button" key={mode} onClick={() => setModuleMode(active, mode as AccessMode)} className="mt-2 flex w-full rounded-lg border px-3 py-2 text-left text-xs hover:bg-slate-50">{label}</button>)}</div><div className="rounded-xl border bg-white p-4 shadow-sm"><h3 className="font-semibold">Permission Legend</h3><p className="mt-2 text-xs text-slate-500">Each feature is stored separately, audited, and applied to the selected organization after saving.</p><p className="mt-3 text-xs text-slate-500">Plan restrictions are defaults. Organization permissions can override them when authorized.</p></div></aside></div>
  </section>;
}
