"use client";

import { useState, useTransition } from "react";
import Link from "next/link";
import { Pencil, Trash2 } from "lucide-react";
import { deleteEmployee } from "@/app/(dashboard)/hrm/actions";

export interface EmployeeRow {
  id: string;
  fullName: string;
  jobTitle: string | null;
  department: string | null;
  monthlySalary: number;
  status: "active" | "inactive";
}

function formatMoney(value: number) {
  return new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
}

export function EmployeesTable({ employees, canManage }: { employees: EmployeeRow[]; canManage: boolean }) {
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);

  function handleDelete(id: string, name: string) {
    if (!confirm(`Remove "${name}" from employees? This can't be undone.`)) return;
    setError(null);
    startTransition(async () => {
      const result = await deleteEmployee(id);
      if (result?.error) setError(result.error);
    });
  }

  if (employees.length === 0) {
    return (
      <div className="rounded-card border border-dashed border-ledger-200 bg-white p-10 text-center dark:border-ledger-700 dark:bg-ink-900">
        <p className="text-sm text-ledger-500 dark:text-ledger-400">No employees yet — add your first one.</p>
      </div>
    );
  }

  return (
    <div className="space-y-3">
      {error && <p className="rounded-md bg-alert-soft px-3 py-2 text-sm text-alert">{error}</p>}

      <div className="overflow-hidden rounded-card border border-ledger-100 bg-white shadow-card dark:border-ledger-700 dark:bg-ink-900">
        <table className="w-full text-sm">
          <thead className="border-b border-ledger-100 text-left text-xs font-medium uppercase tracking-wide text-ledger-400 dark:border-ledger-700">
            <tr>
              <th className="px-4 py-3">Name</th>
              <th className="px-4 py-3">Role</th>
              <th className="px-4 py-3">Department</th>
              <th className="px-4 py-3 text-right">Monthly salary</th>
              <th className="px-4 py-3">Status</th>
              {canManage && <th className="px-4 py-3" />}
            </tr>
          </thead>
          <tbody>
            {employees.map((employee) => (
              <tr key={employee.id} className="border-b border-ledger-50 last:border-0 dark:border-ledger-700/50">
                <td className="px-4 py-3 text-ink-900 dark:text-white">{employee.fullName}</td>
                <td className="px-4 py-3 text-ledger-500 dark:text-ledger-400">{employee.jobTitle ?? "—"}</td>
                <td className="px-4 py-3 text-ledger-500 dark:text-ledger-400">{employee.department ?? "—"}</td>
                <td className="px-4 py-3 text-right figure text-ink-900 dark:text-white">
                  ${formatMoney(employee.monthlySalary)}
                </td>
                <td className="px-4 py-3">
                  <span
                    className={
                      employee.status === "active"
                        ? "rounded-full bg-signal-soft px-2 py-0.5 text-xs font-semibold text-signal"
                        : "rounded-full bg-ledger-100 px-2 py-0.5 text-xs font-semibold text-ledger-500 dark:bg-white/5"
                    }
                  >
                    {employee.status}
                  </span>
                </td>
                {canManage && (
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-3">
                      <Link
                        href={`/hrm/${employee.id}/edit`}
                        className="text-ledger-400 hover:text-ink-900 dark:hover:text-white"
                        aria-label={`Edit ${employee.fullName}`}
                      >
                        <Pencil className="h-3.5 w-3.5" />
                      </Link>
                      <button
                        onClick={() => handleDelete(employee.id, employee.fullName)}
                        disabled={isPending}
                        className="text-ledger-400 hover:text-alert disabled:opacity-40"
                        aria-label={`Remove ${employee.fullName}`}
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  </td>
                )}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}