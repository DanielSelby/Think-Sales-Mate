...
export async function trackOrder(token: string): Promise<TrackedOrder | null> {
  ...
  const { data: order } = await supabase
    ...
    .select("id, org_id, order_number, status, guest_name, delivery_address, delivery_option, delivery_fee, subtotal, total, notes, created_at")
    .eq("access_token", token)
    .maybeSingle();
  if (!order) return null;

  const [{ data: items }, { data: settings }, { data: org }] = await Promise.all([
