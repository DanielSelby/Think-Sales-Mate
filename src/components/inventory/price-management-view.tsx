...
import { updateProductPrice } from "@/app/(dashboard)/inventory/prices/actions";
...
export function PriceManagementView({ products, currency, canManage }: { products: PriceProduct[]; currency: string; canManage: boolean }) {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 pb-12">
    ...
    {tab === "update" && <UpdateTab products={filtered} allProducts={products} currency={currency} query={query} setQuery={setQuery} category={category} setCategory={setCategory} brand={brand} setBrand={setBrand} categories={categories} brands={brands} drafts={drafts} setDrafts={setDrafts} save={save} canManage={canManage} money={money} />}
