    ...
    <div className="min-h-full bg-[#f4f8fc] px-4 py-5 dark:bg-ink-950 md:px-6">
      <div className="mx-auto max-w-[1680px] space-y-5">
        ...
        <div className="grid gap-4 xl:grid-cols-[1.1fr_1.25fr_0.9fr_260px]">
          <section className="rounded-xl border border-[#dce8f2] bg-white shadow-sm dark:border-ledger-700 dark:bg-ink-900"><div className="border-b border-[#e5edf5] p-4"><h2 className="font-bold text-[#17385d] dark:text-white">Cash Summary</h2></div><div className="divide-y divide-[#edf2f7] px-4">{[["Opening Cash", summary.opening], ["Cash Sales", summary.sales], ["MoMo / E-Cash Sales", summary.electronic], ["Total Sales", summary.sales + summary.electronic], ["Customer Debt", -summary.debt], ["Customer Payment (Cash)", summary.customerPayments], ["Refund / Return Sales", -summary.refunds], ["Cash Expenses", -summary.expenses], ["Cash Withdrawals", -summary.withdrawals], ["Cash Deposits", -summary.deposits]].map(([label, value]) => <div key={String(label)} className={`flex items-center justify-between py-3 text-xs ${label === "Total Sales" ? "bg-signal-soft px-2 text-signal" : label === "MoMo / E-Cash Sales" ? "bg-accentTeal-soft px-2 text-accentTeal" : ""}`}><span className={label === "Total Sales" ? "font-semibold text-signal" : label === "MoMo / E-Cash Sales" ? "font-medium text-accentTeal" : "text-ledger-600"}>{label}</span><strong className={Number(value) < 0 ? "text-red-500" : label === "Total Sales" ? "text-signal" : label === "MoMo / E-Cash Sales" ? "text-accentTeal" : "text-[#244e75] dark:text-white"}>{Number(value) < 0 ? "- " : ""}{formatCurrencyAmount(Math.abs(Number(value)), currencyConfig)}</strong></div>)}</div><div className="m-4 flex items-center justify-between rounded-lg bg-[#edf7ff] px-3 py-4"><span className="text-sm font-bold text-[#31577c]">Expected Closing Cash</span><strong className="text-lg text-[#1675d1]">{formatCurrencyAmount(summary.expected, currencyConfig)}</strong></div></section>
