...
export default async function RouteSalesPage() {
  ...
  const [{ data: routes }, { data: customers }, { data: visits }, { data: collections }, { data: sales }, { data: locations }] = await Promise.all([
  ...
  return <RouteSalesWorkspace
    ...
    initial={{ routes: routes ?? [], customers: customers ?? [], visits: visits ?? [], collections: collections ?? [], sales: sales ?? [], locations: locations ?? [] }}
  />;
}
