...
import { decideApproval, type ApprovalDecisionInput } from "@/app/(dashboard)/approvals/actions";
...
export function ApprovalCenter({ rows, currency }: { rows: ApprovalRow[]; currency: string }) {
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_280px]">
        <section className="overflow-hidden rounded-2xl border border-ledger-100 bg-white shadow-card dark:border-ledger-700 dark:bg-ink-900">
          <div className="flex flex-wrap gap-3 border-b border-ledger-100 p-4 dark:border-ledger-700">
            ...
            <button disabled={!selected.length || !!busy} onClick={approveSelected} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"><Check className="h-4 w-4" /> Approve selected</button>
          </div>
