  ...
  return <section className="space-y-5">
    ...
    <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
      ...
      {detailsTab === "permissions" && <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_250px] p-3"><aside className="rounded-xl border bg-white p-3"><h3 className="font-semibold">Modules</h3><div className="mt-3 space-y-1">{visibleModules.map((module) => <button type="button" key={module.name} onClick={() => setActiveModule(module.name)} className={`flex w-full items-center justify-between rounded-lg px-3 py-3 text-left text-xs ${activeModule === module.name ? "bg-blue-600 font-semibold text-white" : "hover:bg-slate-50"}`}><span>{module.name}</span><span>{featureKeys(module).length}</span></button>)}</div></aside>
        ...
        <aside className="space-y-4"><div className="rounded-xl border bg-white p-4"><h3 className="font-semibold">Permission Presets</h3>{[["enabled", "Full Access"], ["read_only", "Read Only"], ["disabled", "Limited Access"]].map(([mode, label]) => <button type="button" key={mode} onClick={() => setModuleMode(active, mode as AccessMode)} className="mt-2 flex w-full rounded-lg border px-3 py-2 text-left text-xs hover:bg-slate-50">{label}</button>)}<button type="button" onClick={applyPlanTemplate} className="mt-2 flex w-full rounded-lg border border-purple-200 bg-purple-50 px-3 py-2 text-left text-xs font-semibold text-purple-700">Apply plan template</button></div><div className="rounded-xl border bg-white p-4"><h3 className="font-semibold">Permission Legend</h3><p className="mt-2 text-xs text-slate-500">Enabled, disabled, read-only, approval, and hidden settings are saved per feature and audited.</p><p className="mt-3 text-xs text-slate-500">Organization rows are explicit overrides of subscription defaults.</p></div></aside></div>}
