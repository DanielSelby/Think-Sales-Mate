...
import { addSupportContact, setSupportContactActive, updateComplaintPriority, updateComplaintStatus } from "./actions";
...
export default function SupportCenter({ complaints, contacts }: { complaints: Complaint[]; contacts: Contact[] }) {
  ...
  return <div className="space-y-5">
    ...
    {contactModal && <ContactForm close={() => setContactModal(false)} />}
  </div>;
}

function ContactForm({ close }: { close: () => void }) {
  const [busy, setBusy] = useState(false);
  const [form, setForm] = useState({ contactType: "support_team" as Contact["contact_type"], name: "", position: "", email: "", phone: "", whatsapp: "", department: "", specialty: "", role: "", assignedRegion: "" });
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><form onSubmit={(event) => { event.preventDefault(); setBusy(true); void addSupportContact(form).then(() => window.location.reload()); }} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h3 className="text-lg font-bold">Add Contact</h3><button type="button" onClick={close}>Close</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{(["name", "position", "department", "specialty", "role", "email", "phone", "whatsapp", "assignedRegion"] as const).map((field) => <input key={field} required={field === "name"} placeholder={field.replace(/([A-Z])/g, " $1")} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} className="h-10 rounded-lg border px-3 text-sm" />)}<select value={form.contactType} onChange={(event) => setForm({ ...form, contactType: event.target.value as Contact["contact_type"] })} className="h-10 rounded-lg border px-3 text-sm"><option value="support_team">Support Team</option><option value="emergency">Emergency</option><option value="technical">Technical</option><option value="sales_subscription">Sales &amp; Subscription</option></select></div><button disabled={busy} className="mt-4 h-10 w-full rounded-lg bg-blue-600 text-sm font-semibold text-white">{busy ? "Saving..." : "Save Contact"}</button></form></div>;
}
