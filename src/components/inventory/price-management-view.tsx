...
export function PriceManagementView({ products, currency, canManage }: { products: PriceProduct[]; currency: string; canManage: boolean }) {
  ...
  function downloadTemplate() {
    const csv = "Product Name,SKU,Barcode,Current Price,New Price,Retail Price,Wholesale Price,VIP Price\n" + products.map((p) => `${p.name},${p.sku},${p.barcode ?? ""},${p.sellingPrice},,${p.sellingPrice},${p.wholesalePrice ?? ""},${p.vipPrice ?? ""}`).join("\n");
