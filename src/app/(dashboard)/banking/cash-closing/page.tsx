...
export default async function CashClosingPage({ searchParams }: { searchParams?: Promise<CashClosingSearchParams> }) {
  ...
  const [closingResult, { data: locations }, { data: currencyRow }, { data: currencySettings }, { data: companyProfile }, { data: members }, summary] = await Promise.all([
    ...
    calculateExpectedCash(today, selectedLocationId, "full_day", effectiveUserId || null).catch(() => ({ opening: 0, sales: 0, discounts: 0, debt: 0, customerPayments: 0, electronic: 0, refunds: 0, expenses: 0, deposits: 0, withdrawals: 0, expected: 0 })),
  ]);
