...
function isDataChange(record: AuditRecord) {
  ...
  return Boolean(
    ...
    /\b(create|created|add|added|update|updated|edit|edited|delete|deleted|remove|removed|adjust|adjusted|transfer|transferred|approve|approved|reject|rejected|void|refun[ded]+|receive|received|import|merge)\b/.test(value),
  );
}
...
function recordBranch(record: AuditRecord) {
  return textValue(record.metadata.branch_name) || textValue(record.metadata.branch_id);
}
