  ...
  return (
    <div className="communication-workspace relative flex h-[calc(100vh-6.5rem)] min-h-[620px] min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white pt-14 shadow-sm">
      <div className="absolute inset-x-0 top-0 z-20 flex gap-1 overflow-x-auto border-b border-slate-200 bg-white p-2 shadow-sm">
        ...
        <button onClick={() => setWorkspaceTab("announcements")} className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "announcements" ? "" : "text-slate-500"}`} style={workspaceTab === "announcements" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}><Megaphone className="h-3 w-3" />Announcements</button>
        <button onClick={() => setWorkspaceTab("templates")} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "templates" ? "" : "text-slate-500"}`} style={workspaceTab === "templates" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>Message Templates</button>
        <button onClick={() => setWorkspaceTab("automations")} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "automations" ? "" : "text-slate-500"}`} style={workspaceTab === "automations" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>Automated Messages</button>
        <button onClick={() => setWorkspaceTab("history")} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "history" ? "" : "text-slate-500"}`} style={workspaceTab === "history" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>Message History</button>
        <button onClick={() => setWorkspaceTab("calls")} className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "calls" ? "" : "text-slate-500"}`} style={workspaceTab === "calls" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}><Phone className="h-3 w-3" />Calls History</button>
        <button onClick={() => setWorkspaceTab("approvals")} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "approvals" ? "" : "text-slate-500"}`} style={workspaceTab === "approvals" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>Template Approvals</button>
        <button onClick={() => setWorkspaceTab("template-analytics")} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${workspaceTab === "template-analytics" ? "" : "text-slate-500"}`} style={workspaceTab === "template-analytics" ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>Template Analytics</button>
      </div>
      ...
      <aside className="flex w-[300px] shrink-0 flex-col border-r border-slate-200 bg-white">
        <div className="border-b border-slate-100 p-4">
          ...
          <div className="mt-3 flex gap-1 overflow-x-auto pb-1">{tabs.map((tab) => <button key={tab} onClick={() => setFilter(tab)} className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-[10px] font-semibold ${filter === tab ? "" : "text-slate-500 hover:bg-slate-50"}`} style={filter === tab ? { background: theme.colors.primaryPale, color: theme.colors.primary } : undefined}>{tab}{tab === "Unread" && unreadCount > 0 && <span className="ml-1 rounded-full bg-red-500 px-1.5 py-0.5 text-[9px] text-white">{unreadCount}</span>}</button>)}</div>
