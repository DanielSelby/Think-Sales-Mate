...
export default async function StockRequestHistoryPage({ searchParams }: { searchParams?: { id?: string } }) {
  ...
  return <StockRequestHistory initialRequestId={searchParams?.id} canApprove={can(context.role, "inventory.stock_request.approve")} requests={visibleRequests.map((request) => ({ id: request.id, label: `REQ-${String(request.request_number).padStart(4, "0")}`, status: request.status, source: locationNames.get(request.source_location_id) ?? "—", destination: locationNames.get(request.requesting_location_id) ?? "—", createdAt: request.created_at, totalQuantity: quantities.get(request.id) ?? 0, transferId: request.transfer_id, items: (items ?? []).filter((item) => item.request_id === request.id).map((item) => { const product = Array.isArray(item.products) ? item.products[0] : item.products; return { productName: product?.name ?? "Unknown product", sku: product?.sku ?? null, quantity: item.quantity, reason: item.reason }; }) }))} />;
}
