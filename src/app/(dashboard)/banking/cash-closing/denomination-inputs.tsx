      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
        {DEFAULT_DENOMINATIONS.map((denomination) => (
          <label key={denomination} className="text-[10px]">
            ...
            <input name={`denomination_${denomination}`} type="number" min="0" step="1" defaultValue="0" onChange={update(`denomination_${denomination}`, denomination)} className="mt-1 h-8 w-full rounded border border-ledger-200 px-1 dark:border-ledger-700 dark:bg-ink-950" />
          </label>
        ))}
        {customRows.map((rowId) => (
          <div key={rowId} className="col-span-2 grid grid-cols-2 gap-1">
            <label className="text-[10px]">Value<input name={`custom_denomination_value_${rowId}`} type="number" min="0.01" step="0.01" onChange={(event) => setValues((current) => ({ ...current, [`custom_${rowId}`]: Number(event.target.value || 0) * (Number((document.querySelector(`input[name="custom_denomination_quantity_${rowId}"]`) as HTMLInputElement | null)?.value || 0)) }))} className="mt-1 h-8 w-full rounded border border-ledger-200 px-1 dark:border-ledger-700 dark:bg-ink-950" required /></label>
            <label className="text-[10px]">Qty<input name={`custom_denomination_quantity_${rowId}`} type="number" min="0" step="1" defaultValue="0" onChange={(event) => setValues((current) => ({ ...current, [`custom_${rowId}`]: Number((document.querySelector(`input[name="custom_denomination_value_${rowId}"]`) as HTMLInputElement | null)?.value || 0) * Math.max(0, Math.floor(Number(event.target.value) || 0)) }))} className="mt-1 h-8 w-full rounded border border-ledger-200 px-1 dark:border-ledger-700 dark:bg-ink-950" /></label>
            <span className="col-span-2 text-right text-[10px] text-ledger-500">Amount: {formatCurrencyAmount(values[`custom_${rowId}`] ?? 0, currency)}</span>
          </div>
        ))}
      </div>
