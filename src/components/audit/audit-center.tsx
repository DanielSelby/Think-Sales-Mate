...
export function AuditCenter({ records }: { records: AuditRecord[] }) {
  ...
  }), [action, from, module, query, records, selectedUser, severity, status, tab, to]);
  ...
  const clearFilters = () => { setQuery(""); setModule("all"); setAction("all"); setStatus("all"); setSeverity("all"); setFrom(""); setTo(""); setSelectedUser(""); setSelectedIds([]); setPage(1); };
  const saveView = () => {
