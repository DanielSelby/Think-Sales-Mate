...
export function DuplicateReviewCenter({ rows }: { rows: ReviewRow[] }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-ledger-100 bg-white shadow-card dark:border-ledger-700 dark:bg-ink-900">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[1220px] table-fixed text-left text-sm"><colgroup><col className="w-[23%]" /><col className="w-[14%]" /><col className="w-[25%]" /><col className="w-[10%]" /><col className="w-[9%]" /><col className="w-[11%]" /><col className="w-[8%]" /></colgroup><thead className="bg-ledger-50 text-[10px] uppercase tracking-wide text-ledger-500 dark:bg-ink-950"><tr><th className="px-5 py-3">Products</th><th className="px-3 py-3">SKU</th><th className="px-3 py-3">Locations</th><th className="px-3 py-3">Brand</th><th className="px-3 py-3">Similarity</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Actions</th></tr></thead><tbody className="divide-y divide-ledger-100 dark:divide-ledger-700">{visibleRows.map((row) => <tr key={row.id}><td className="px-5 py-5 align-top font-semibold leading-6">{row.name}</td><td className="px-3 py-5 align-top font-mono text-xs text-ledger-500">{row.sku}</td><td className="px-3 py-5 align-top font-display text-sm font-bold leading-6 text-ink-800 dark:text-white">{row.locations}</td><td className="px-3 py-5 align-top text-xs">{row.brand ?? "—"}</td><td className="px-3 py-5 align-top"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${row.score === 100 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>{row.score}%</span></td><td className="px-3 py-5 align-top text-xs text-ledger-500">{row.type === "barcode" ? `Barcode: ${row.barcode}` : row.type === "brand_model" ? "Brand + model candidate" : "Needs review"}</td><td className="px-3 py-5 align-top"><div className="flex flex-col gap-2"><Link href={`/inventory/merge?ids=${row.id.split("-")[0]}`} className="inline-flex items-center justify-center gap-1 rounded-lg bg-emerald-600 px-2 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-700"><GitMerge className="h-3 w-3" /> Merge</Link><button type="button" onClick={() => setActiveTab(activeTab)} className="rounded-lg border border-red-200 bg-red-50 px-2 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100">Ignore</button></div></td></tr>)}</tbody></table></div>
